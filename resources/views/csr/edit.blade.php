...
@section('title', 'Edit Program CSR')
...
<div class="max-w-5xl mx-auto mt-12 bg-white shadow-lg rounded-2xl border border-gray-200 p-8"
     ...
     x-init="
        ...
        cekSisaAnggaran();
     ">
     <h2 class="text-2xl font-bold text-gray-800 mb-6 border-b pb-2">✏️ Edit Program CSR</h2>


    <div class="grid md:grid-cols-3 gap-6">
        <!-- Form Input -->
        <form method="POST" action="{{ route('csr.update', $csr->id) }}" class="space-y-5 md:col-span-2">
            @csrf
            ...
            <!-- Pemegang Saham -->
            <div class="relative" x-data="{ open: false, pemegang_saham: '{{ $defaultPemegangSaham }}' }">
        ...
        <div>
            ...
            @php
            $isEdit = isset($csr);
            $defaultBidang = $isEdit ? $csr->bidang_kegiatan : '';
        @endphp
        
        <div>
            <div x-data="{ open: false, selectedBidang: '{{ $defaultBidang }}' }" class="relative">
    ...
    <div class="relative inline-flex w-full">
        ...
        <div x-show="open" @click.away="open = false"
            ...
            @php
            $isEdit = isset($csr);
            $defaultBulan = $isEdit ? $csr->bulan : '';
            $defaultBulanLabel = $defaultBulan ? DateTime::createFromFormat('!m', $defaultBulan)->format('F') : '';
        @endphp
        ...
        <div>
            <div x-data="{ 
                    ...
                    selectedBulanLabel: '{{ $defaultBulanLabel }}' 
                }" class="relative">
...
@php
    $isEdit = isset($csr);
    $realisasi = $isEdit ? (int) $csr->realisasi_csr : '';
@endphp
...
>
    ...
    <input type="hidden" name="realisasi_csr" :value="raw">
            ...
            <div>
                ...
                <textarea name="ket" rows="3" class="w-full px-4 py-2 border rounded-lg">{{ old('ket', $csr->ket ?? '') }}</textarea>
            </div>
